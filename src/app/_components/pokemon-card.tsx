...
const PokemonCard: React.FC<PokemonCardProps> = ({
    ...
    return (
        <div key={id} data-aos="fade-up" className={`border overflow-y-hidden rounded-lg w-full py-5 px-3 relative group`} onMouseEnter={() => setIsHovered(true)}
            ...
            <div className="justify-between group-hover:-translate-y-10 transition-all duration-500 bottom-3">
                <p className={`sm:text-[16px]`}>{String(id).padStart(3, '0')}</p>
                <p className="sm:text-[18px]">{CapitalizeString(name)}</p>
            </div>
            <div className="absolute top-[100%] left-0 right-0 mx-3 group-hover:-translate-y-[50px] duration-500 ease-in-out">
                <Link href="">
